...
5. Configurá un SMTP propio en **Authentication → SMTP Settings** antes de invitar un negocio real. El intento temporal con Gmail y contraseña de aplicación del 2026-09-02 devolvió `Gateway Timeout`; la cuenta de prueba no llegó a crearse y no se guardaron credenciales en el proyecto. Para avanzar gratis en pruebas se recomienda **Brevo Free** mediante su SMTP Relay (300 correos diarios); verificá un remitente antes de usarlo. Para producción se migrará a **Resend Free** con un dominio propio. El SMTP gratuito predeterminado de Supabase solo entrega a miembros del equipo y limita los correos, por lo que no es suficiente para administradores externos.
