-- Tope de unidades por pedido, a elección de cada negocio.
--
-- Hasta hoy el único tope era el general —30 renglones de hasta 99 unidades—,
-- igual para todos. Una tienda con poco stock de algo que se agota rápido no
-- tenía cómo evitar que un solo pedido se lo apartara entero durante el tiempo
-- de la reserva. Ahora el dueño puede poner su propio máximo de unidades.
--
-- Vacío es «sin tope propio», y así queda todo negocio que ya existe: nada
-- cambia para quien no lo toca. El número va de 1 a 2970 (30 × 99): más que eso
-- no se puede pedir de todos modos.
--
-- Vive en la base, como los demás topes: la ruta se puede saltar con la clave
-- privilegiada; `crear_pedido_reservado`, no. Es la función de
-- `20261027090000_tope_de_pedidos_por_ip.sql` con esa única comprobación
-- agregada, que responde `TOPE_UNIDADES`.

alter table public.negocios
  add column tope_unidades_pedido integer
    check (tope_unidades_pedido between 1 and 2970);

comment on column public.negocios.tope_unidades_pedido is
  'Máximo de unidades por pedido que acepta el negocio. Vacío: sin tope propio.';

-- El catálogo lo lee para avisar en el carrito antes de enviar; el dueño lo
-- edita desde «Atención y reservas».
grant select (tope_unidades_pedido) on table public.negocios to anon, authenticated;
grant update (tope_unidades_pedido) on table public.negocios to authenticated;
...
begin
  ...
  -- El tope de unidades del negocio, si puso uno. Cuenta todas las unidades del
  -- pedido, no los renglones: tres camisas M y dos L son cinco. Va antes del
  -- contador por IP, porque es un pedido mal armado y no un intento de más.
  if v_negocio.tope_unidades_pedido is not null then
    select sum(solicitado.cantidad) into v_unidades
    from jsonb_to_recordset(p_items) as solicitado(cantidad integer);

    if v_unidades > v_negocio.tope_unidades_pedido then
      raise exception using errcode = 'P0001', message = 'TOPE_UNIDADES';
    end if;
  end if;
  ...
  -- Quince por IP y por negocio cada quince minutos (antes, cinco). Desde que
  -- Turnstile exige verificar a una persona, este tope es la segunda línea; y
  -- con cinco, las redes móviles —que ponen a muchos clientes detrás de una
  -- misma IP— podían dejar afuera a clientes reales de un negocio concurrido.
  if v_limite > 15 then
