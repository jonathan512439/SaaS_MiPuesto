-- El tope de pedidos por IP pasa de 5 a 15 cada quince minutos, por negocio.
--
-- Cinco era la defensa principal contra un programa que apartara stock sin
-- comprar. Desde el 2026-09-24 esa defensa es Cloudflare Turnstile, que exige
-- verificar a una persona antes de crear el pedido; el tope por IP queda de
-- segunda línea. Y con cinco, un negocio con muchos pedidos en la hora pico
-- podía rechazar a clientes reales: las compañías móviles de Bolivia ponen a
-- muchos usuarios detrás de pocas IP.
--
-- Es la misma función de `20261025090000_el_nombre_vuelve_a_ser_opcional.sql`
-- con esa única cifra cambiada. La ruta de turnos usa el mismo número
-- (`TOPE_INTENTOS_POR_VENTANA`), y `las-citas-publicas-tienen-tope.test.ts`
-- compara los dos. Cada vez que alguien choca con el tope queda anotado en el
-- registro del Worker, para saber si alcanza.
